...
# @typescript/typescript-<platform> binary may be missing because npm only installs
...
function ensure_host_tsgo()
{
	...
	local host_pkg="@typescript/typescript-${host_platform}"
	local host_pkg_dir="node_modules/${host_pkg}"
	...
	local tsgo_version
	tsgo_version="$(node -p "require('./node_modules/@typescript/native/package.json').optionalDependencies['${host_pkg}']")"
