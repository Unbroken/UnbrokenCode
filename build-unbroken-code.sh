...
# After a cross-compile `npm ci --cpu=<target>`, the host platform's
# @typescript/native-preview binary may be missing because npm only installs
# optional dependencies matching the --cpu target.  tsgo runs on the *host*
# during the build, so we need to ensure the host binary is present.
function ensure_host_tsgo()
{
	local host_platform
	host_platform="$(node -p "process.platform + '-' + process.arch")"
	local host_pkg="@typescript/native-preview-${host_platform}"
	local host_pkg_dir="node_modules/@typescript/native-preview-${host_platform}"

	if [ -d "$host_pkg_dir" ]; then
		return 0
	fi

	echo "Installing host tsgo binary ($host_pkg) for cross-compile build..."
	local tsgo_version
	tsgo_version="$(node -p "require('./node_modules/@typescript/native-preview/package.json').optionalDependencies['${host_pkg}']")"
	# Extract the package manually instead of `npm install --no-save`: a plain
	# npm install re-reifies the tree with the *host* platform context, pruning
	# the optional dependencies that `npm ci --cpu=<target>` just installed for
	# the target arch (e.g. @vscode/vsce-sign-<target>, @parcel/watcher-<target>)
	# and replacing them with host-arch variants, which corrupts the packaged app.
	local pack_dir
	pack_dir="$(mktemp -d)"
	(cd "$pack_dir" && npm pack "${host_pkg}@${tsgo_version}" >/dev/null 2>&1)
	mkdir -p "$host_pkg_dir"
	tar -xzf "$pack_dir"/*.tgz -C "$host_pkg_dir" --strip-components=1
	rm -rf "$pack_dir"
}
...
function Update_Version()
{
	...
	# Update engines.vscode in extensions/copilot/package.json to match the root version.
	# Use a targeted string replacement to avoid reformatting the rest of the file.
	if [ -f "extensions/copilot/package.json" ]; then
		echo "Updating extensions/copilot/package.json engines.vscode to ^${NEW_VERSION_STRING}..."
		NEW_VERSION_STRING="${NEW_VERSION_STRING}" node -e "
			const fs = require('fs');
			const path = 'extensions/copilot/package.json';
			const newVersion = process.env.NEW_VERSION_STRING;
			const original = fs.readFileSync(path, 'utf8');
			const enginesMatch = original.match(/\"engines\"\s*:\s*\{[^}]*\}/);
			if (!enginesMatch) {
				console.error('Could not find engines block in ' + path);
				process.exit(1);
			}
			const updatedEngines = enginesMatch[0].replace(/(\"vscode\"\s*:\s*\")[^\"]*(\")/, '\$1^' + newVersion + '\$2');
			if (updatedEngines === enginesMatch[0]) {
				console.error('Could not find engines.vscode entry in ' + path);
				process.exit(1);
			}
			const updated = original.replace(enginesMatch[0], updatedEngines);
			if (updated !== original) {
	...
	echo "  git add package.json package-lock.json unbroken.version extensions/copilot/package.json"
...
function Build_macOS()
{
	...
	if $DO_BUILD; then
		...
		if $BUILD_MACOS_X64; then
			...
			rm -rf "$DIST_DIR/VSCode-darwin-x64/Unbroken Code.app/Contents/Resources/app/extensions/copilot/dist/test-"*
			rm -rf "$DIST_DIR/VSCode-darwin-x64/Unbroken Code.app/Contents/Resources/app/extensions/copilot/dist/sanity-test-"*
			rm -rf "$DIST_DIR/VSCode-darwin-x64/Unbroken Code.app/Contents/Resources/app/node_modules/@parcel/watcher-darwin-arm64"
		fi
		...
		if $BUILD_MACOS_ARM64; then
			...
			rm -rf "$DIST_DIR/VSCode-darwin-arm64/Unbroken Code.app/Contents/Resources/app/extensions/copilot/dist/test-"*
			rm -rf "$DIST_DIR/VSCode-darwin-arm64/Unbroken Code.app/Contents/Resources/app/extensions/copilot/dist/sanity-test-"*
			rm -rf "$DIST_DIR/VSCode-darwin-arm64/Unbroken Code.app/Contents/Resources/app/node_modules/@parcel/watcher-darwin-x64"
		fi
