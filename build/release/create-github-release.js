...
async function getPreviousReleaseNotesSubjects(octokit, tag) {
    const subjects = new Set();
    try {
...
async function generateReleaseNotes(octokit, buildCommit, currentTag) {
...
async function main() {
    ...
    const architectures = ['arm64', 'x64', 'universal'];
    const foundVersions = [];
    // Check for built product.json in all architecture builds (macOS and Windows)
    for (const arch of architectures) {
        // Check macOS builds
        const macOSProductPath = path.join(distDir, `VSCode-darwin-${arch}`, 'Unbroken Code.app', 'Contents', 'Resources', 'app', 'product.json');
        if (fs.existsSync(macOSProductPath)) {
    ...
    if (!hasDarwinBuilds && !hasWindowsBuilds) {
    ...
    if (hasWindowsBuilds) {
        ...
        for (const arch of windowsArchitectures) {
            ...
            if (!fs.existsSync(archivePath)) {
                ...
                (0, child_process_1.execSync)(`cd "${distDir}" && tar -czf "${archiveName}" "VSCode-win32-${arch}"`, {
                    stdio: 'inherit'
                });
            }
            assets.push({
    ...
    if (hasDarwinBuilds) {
        releaseBodyParts.push('**macOS**: Download the DMG file for your architecture:', '- Apple Silicon: `UnbrokenCode-darwin-arm64-*.dmg`', '- Intel: `UnbrokenCode-darwin-x64-*.dmg`', '- Universal (works on both): `UnbrokenCode-darwin-universal-*.dmg`', '', 'Open the DMG and drag Unbroken Code to your Applications folder.', '', '#### Recommended: Disable Font Smoothing for Pixel-Perfect Rendering', 'For the best experience with Unbroken Code\'s crisp font rendering, disable font smoothing:', '', '```bash', 'defaults -currentHost write -g AppleFontSmoothing -int 0', '```', '', 'To re-enable: `defaults -currentHost delete -g AppleFontSmoothing`', '', '**Why?** Apple\'s font smoothing makes text appear bold and blurry. [Learn more](https://tonsky.me/blog/monitors/#turn-off-font-smoothing)', '');
    }
    if (hasWindowsBuilds) {
        releaseBodyParts.push('**Windows**: Choose your installation method:', '', '#### Installer (Recommended)', '- **User Install** (no admin required): `UnbrokenCodeSetup-{arch}-user-*.exe`', '- **System Install** (requires admin): `UnbrokenCodeSetup-{arch}-system-*.exe`', '', 'Architecture:', '- x64: For Intel/AMD 64-bit processors', '- arm64: For ARM-based Windows devices', '', '#### Portable ZIP', 'For a portable installation, download:', '- `UnbrokenCode-win32-x64-*.zip` (Intel/AMD)', '- `UnbrokenCode-win32-arm64-*.zip` (ARM)', '', 'Extract the ZIP and run `Code.exe` from the extracted folder.', '');
    }
    releaseBodyParts.push('### Auto-Update', 'This release supports automatic updates. Once installed, Unbroken Code will check for updates automatically.');
