...
function isCommitOnUpstream(commit) {
    if (!remoteExists('upstream')) {
        return false;
    }
    try {
        const output = (0, child_process_1.execSync)(`git branch -r --list 'upstream/*' --contains ${commit}`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
...
async function main() {
    ...
    if (showReleaseNotes) {
        ...
        const buildCommit = (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf8' }).trim();
