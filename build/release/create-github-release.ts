...
async function main() {
	...
	const getCliDownloadLink = (platform: string, arch: string): string => {
	...
	if (hasManifestLinuxBuilds) {
		...
		if (x64DebAsset || arm64DebAsset) {
			...
			linuxSections.push(
				'',
				'Install with: `sudo dpkg -i UnbrokenCode-linux-*.deb`'
			);
		}
		...
		if (x64RpmAsset || arm64RpmAsset) {
			linuxSections.push(
				...
				// allow-any-unicode-next-line
				'### 📦 RedHat/Fedora (.rpm)',
				'For RedHat, Fedora, SUSE, and derivatives:'
			);
			...
			linuxSections.push(
				'',
				'Install with: `sudo rpm -i UnbrokenCode-linux-*.rpm`'
			);
		}
		...
		if (x64TarAsset || arm64TarAsset) {
			...
			linuxSections.push(
				'',
				'Extract with: `tar -xzf UnbrokenCode-linux-*.tar.gz`',
				'Run with: `./UnbrokenCode-linux-*/bin/code`'
			);
		}
