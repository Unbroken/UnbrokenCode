...
(function () {
	...
	function resolveProcessPriority(command: string, args: string[]): string {
	...
	cp.spawn = function (command: string, argsOrOptions?: any, maybeOptions?: any) {
		...
		const priority = resolveProcessPriority(command, args);
		...
		if (priority !== 'default' && process.platform === 'darwin') {
			...
			if (options?.shell) {
				return originalSpawn.call(this, `taskpolicy -c ${qosClass} ${command}`, args, options);
			} else {
				return originalSpawn.call(this, 'taskpolicy', ['-c', qosClass, command, ...args], options);
			}
		}
	...
	cp.fork = function (modulePath: string, argsOrOptions?: any, maybeOptions?: any) {
		const { args, options } = normalizeSpawnArgs(argsOrOptions, maybeOptions);
		const priority = resolveProcessPriority(modulePath, args);
