...
suite('MarkersFilterOptions', () => {
	...
	test('source filter', () => {
		const filterOptions = new FilterOptions('@source:ts', [], true, true, true, true, uriIdentityService);
	...
	test('source filter with negation', () => {
		const filterOptions = new FilterOptions('!@source:eslint', [], true, true, true, true, uriIdentityService);
	...
	test('multiple source filters', () => {
		const filterOptions = new FilterOptions('@source:eslint @source:ts', [], true, true, true, true, uriIdentityService);
	...
	test('source filter combined with text filter', () => {
		const filterOptions = new FilterOptions('@source:ts error', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['ts']);
		assert.deepStrictEqual(filterOptions.excludeSourceFilters, []);
	...
	test('no source filter when not specified', () => {
		const filterOptions = new FilterOptions('some text', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, []);
		assert.deepStrictEqual(filterOptions.excludeSourceFilters, []);
	...
	test('source filter case insensitive', () => {
		const filterOptions = new FilterOptions('@SOURCE:TypeScript', [], true, true, true, true, uriIdentityService);
	...
	test('complex filter with multiple source filters and text', () => {
		const filterOptions = new FilterOptions('text1 @source:eslint @source:ts text2', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['eslint', 'ts']);
		assert.deepStrictEqual(filterOptions.excludeSourceFilters, []);
	...
	test('source filter at the beginning', () => {
		const filterOptions = new FilterOptions('@source:eslint foo', [], true, true, true, true, uriIdentityService);
	...
	test('source filter at the end', () => {
		const filterOptions = new FilterOptions('foo @source:eslint', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['eslint']);
		assert.strictEqual(filterOptions.textFilter.text, 'foo');
	});

	test('source filter in the middle', () => {
		const filterOptions = new FilterOptions('foo @source:eslint bar', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['eslint']);
	...
	test('multiple consecutive source filters', () => {
		const filterOptions = new FilterOptions('@source:eslint @source:ts foo', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['eslint', 'ts']);
		assert.strictEqual(filterOptions.textFilter.text, 'foo');
	});
	...
	test('single quoted source with spaces', () => {
		const filterOptions = new FilterOptions('@source:"hello world"', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['hello world']);
		assert.strictEqual(filterOptions.textFilter.text, '');
	});

	test('quoted source combined with text filter', () => {
		const filterOptions = new FilterOptions('@source:"hello world" foo', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['hello world']);
		assert.strictEqual(filterOptions.textFilter.text, 'foo');
	});

	test('mixed quoted and unquoted sources (OR logic)', () => {
		const filterOptions = new FilterOptions('@source:"hello world" @source:eslint @source:ts', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['hello world', 'eslint', 'ts']);
	});

	test('multiple quoted sources (OR logic)', () => {
		const filterOptions = new FilterOptions('@source:"hello world" @source:"foo bar"', [], true, true, true, true, uriIdentityService);
	...
	test('quoted source in the middle of filter', () => {
		const filterOptions = new FilterOptions('foo @source:"hello world" bar', [], true, true, true, true, uriIdentityService);
		assert.deepStrictEqual(filterOptions.includeSourceFilters, ['hello world']);
		assert.strictEqual(filterOptions.textFilter.text, 'foo bar');
	});

	test('complex filter with quoted and unquoted mixed', () => {
		const filterOptions = new FilterOptions('@source:"TypeScript Compiler" @source:eslint !@source:"My Extension" text', [], true, true, true, true, uriIdentityService);
	...
	test('no filters - always matches', () => {
		const filterOptions = new FilterOptions('foo', [], true, true, true, true, uriIdentityService);
	...
	test('OR logic with negation', () => {
		const filterOptions = new FilterOptions('@source:eslint @source:ts !@source:error', [], true, true, true, true, uriIdentityService);
		assert.strictEqual(filterOptions.matchesSourceFilters('eslint'), true);
		assert.strictEqual(filterOptions.matchesSourceFilters('ts'), true);
