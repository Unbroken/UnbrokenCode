...
export class TerminalInstance extends Disposable implements ITerminalInstance {
	...
	private _bellNotification: dom.INotification | undefined;
	...
	private async _showBellNotification(): Promise<void> {
		...
		if (this._bellNotification && this._bellNotificationTitle === title) {
			return;
		}

		// Dispose the previous notification if present.
		if (this._bellNotification) {
			this._bellNotification.dispose();
			this._bellNotification = undefined;
		...
		const notification = await dom.triggerNotification(
			nls.localize('terminalBellNotification', "Bell in terminal: {0}", title),
			{ tag: `terminal-bell-${this._bellNotificationId}` }
		);

		if (notification) {
			this._bellNotification = notification;
			this._bellNotificationTitle = title;
			this._register(Event.once(notification.onClick)(async () => {
			...
			this._register(Event.once(notification.onDidClose)(() => {
				this._bellNotification = undefined;
				this._bellNotificationTitle = undefined;
			}));
		}
